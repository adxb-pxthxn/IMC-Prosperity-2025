...
class JamStrategy(Strategy):
    def __init__(self, symbol: str, limit: int):
        super().__init__(symbol, limit)
        self.prices = deque(maxlen=20)  # configurable window
        self.alpha = 0.2  # signal threshold, tune later

    def update_signal(self, price: float) -> float:
        self.prices.append(price)
        if len(self.prices) < 2:
            return 0
        return price - np.mean(self.prices)  # simple momentum signal
    ...
    def act(self, state: TradingState, traderObject) -> None:
        ...
        if signal > self.alpha and position < self.limit:
...
class Basket1Strat(Strategy):
    ...
    def act(self,state,traderObject):
        ...
        cros,jams,dj=self.get_mid_price(state.order_depths['CROISSANTS']),self.get_mid_price(state.order_depths['JAMS']),self.get_mid_price(state.order_depths['DJEMBES'])
