...
class macaronStrategy(Strategy):
    def act(self, state, traderObject=None):
        # Estimate the fair value of the product (fundamental value based on theoretical or external inputs)
        fair_price = traderObject.fair_price if traderObject else 1000  # default fallback
        next_price_move = traderObject.next_price_move if traderObject else 0  # predicted market trend

        # Retrieve cost-related observations for the asset "MAGNIFICENT_MACARNONS"
        macaroonObservation = state.observations.conversionObservations["MAGNIFICENT_MACARNONS"]

        # Total cost of selling includes transport and import costs
        cost = macaroonObservation.transportFees + macaroonObservation.importTariff

        position_limit = self.limit  # how many units this strategy is allowed to trade

        # Begin optimization for ask (selling) price
        my_ask = state.maxamt_bidprc  # starting price to consider selling at (top of book)
        ask_max_expected_profit = 0
        optimal_my_ask = 1e9  # Initialize to a high value to be minimized

        # Iterate through potential ask prices until fair_price
        while my_ask < fair_price:
            delta = my_ask - fair_price  # how far our ask is above fair value
            execution_prob = ExecutionProb.orchids(delta)  # lower delta => higher chance of execution

            # If our price is better than best bid, we’ll need to estimate chance of execution
            if my_ask > state.best_bid:
                # Profit is the markup over the market buy price minus any projected adverse movement
                trading_profit = my_ask - (state.orchid_south_askprc + next_price_move)
                expected_profit = execution_prob * (trading_profit - cost)
            else:
                # If our ask is already below market bids, we're likely to get filled immediately
                execution_prob_list = []
                price_list = []
                amount_list = []
                ...
                for price, amount in state.bids:
                    if price >= my_ask:
                        execution_prob_list.append(1)  # certainty of fill
                        price_list.append(price)
                        amount_list.append(amount)

                total_amount = np.sum(amount_list)

                # If there's still capacity left, simulate partial fill at our ask with prob < 1
                if total_amount < LIMITS["MAGNIFICENT_MACARONS"]:
                    execution_prob_list.append(ExecutionProb.orchids(delta))
                    price_list.append(my_ask)
                    amount_list.append(position_limit - total_amount)

                trading_profit_list = np.array(price_list) - (state.orchid_south_askprc + next_price_move)
                expected_profit = (np.array(execution_prob_list) * (trading_profit_list - cost) * 
                                   np.array(amount_list) / position_limit).sum()

            # Track best expected profit and corresponding price
            if expected_profit > ask_max_expected_profit:
                optimal_my_ask = my_ask
                ask_max_expected_profit = expected_profit

            my_ask += 1  # move to next price level

        # Now optimize bid (buying) strategy
        cost = macaroonObservation.transportFees + macaroonObservation.exportTariff + macaroonObservation.storageFees

        my_bid = state.maxamt_askprc  # start with top ask
        ...
        while my_bid > fair_price:
            ...
            if my_bid < state.best_ask:
                # Buying below fair market value — profit is resale value minus cost
                trading_profit = (state.orchid_south_bidprc + next_price_move) - my_bid
                expected_profit = execution_prob * (trading_profit - cost)
            else:
                # If bidding inside the book — high chance of immediate execution
                execution_prob_list = []
                price_list = []
                amount_list = []
                ...
                for price, amount in state.asks:
                    if price <= my_bid:
                        ...
                        price_list.append(price)
                        amount_list.append(abs(amount))  # sell volumes are negative

                total_amount = np.sum(amount_list)

                # Add additional potential execution at our bid
