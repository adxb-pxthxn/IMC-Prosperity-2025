...
class JamStrategy(Strategy):
    ...
    def act(self, state: TradingState, traderObject) -> None:
        ...
        if mid_price is None: # dont trade if order-book is empty on either side
            return
...
class DjembeStrategy(Strategy):
    def __init__(self, symbol, limit, window=50, z_entry=2.0, z_exit=0.5):
        super().__init__(symbol, limit)
        self.window = deque(maxlen=window)
        self.z_entry = z_entry
        self.z_exit = z_exit

    def git_fair_spread(self, order_depth):
    ...
    def get_fair_spread(self, djembe_price, basket1_price):
        spread = basket1_price - djembe_price
        ...
        return spread, mean, z_score

    def act(self, state: TradingState, traderObject) -> None:
        order_depth_djembe = state.order_depths.get("DJEMBES")
        order_depth_basket = state.order_depths.get("PICNIC_BASKET1")
        if not order_depth_djembe or not order_depth_basket:
            return

        djembe_mid = self.get_mid_price(order_depth_djembe)
        basket_mid = self.get_mid_price(order_depth_basket)
        if djembe_mid is None or basket_mid is None:
            return

        spread, mean, z = self.get_fair_spread(djembe_mid, basket_mid)
        position = state.position.get("DJEMBES", 0)
        best_bid = max(order_depth_djembe.buy_orders.keys(), default=None)
        best_ask = min(order_depth_djembe.sell_orders.keys(), default=None)

        # Entry signals
        if z > self.z_entry and position > -self.limit:
            if best_bid is not None:
                self.sell(best_bid, min(self.limit + position, 10))
        elif z < -self.z_entry and position < self.limit:
            if best_ask is not None:
                self.buy(best_ask, min(self.limit - position, 10))
        # Exit signal
        elif abs(z) < self.z_exit and position != 0:
    ...
    def save(self):
        return {"spread_window": list(self.window)}

    def load(self, data):
        self.window = deque(data.get("spread_window", []), maxlen=self.window.maxlen)
