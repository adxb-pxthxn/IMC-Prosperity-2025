...
class MeanReversion(MarketMaking):
    ...
    def act(self, state, traderObject):
        ...
        try:
            best_ask_fair = min([p for p in sell_orders if p > fair + 1], default=fair + 1)
        except ValueError:
            best_ask_fair = fair + 1

        try:
            best_bid_fair = max([p for p in buy_orders if p < fair + 1], default=fair - 1)
        except ValueError:
...
class InkStrategy(MeanReversion):
    ...
    def get_fair_price(self, order, traderObject):
        ...
        if price is None:
            # Safe fallback if mid price couldn't be computed
            return (
                self.emv.price_ema.value or 0,
                self.emv.deviation_ema.value or 1,
                self.emv.long_ema.value or 0
            )
        return self.emv.update(price)

    def act(self, state, traderObject):
        order_depth = state.order_depths[self.symbol]
        fair, dev, long = self.get_fair_price(order_depth, traderObject)

        # Override base act to call MR logic with only fair
        self._mean_reversion_act(state, fair)

    def _mean_reversion_act(self, state, fair_price):
        buy_volume = 0
        sell_volume = 0
        order_depth = state.order_depths[self.symbol]
        ...
        try:
            best_ask_fair = min([p for p in sell_orders.keys() if p > fair_price + 1], default=fair_price + 1)
        except ValueError:
            best_ask_fair = fair_price + 1

        try:
            best_bid_fair = max([p for p in buy_orders.keys() if p < fair_price + 1], default=fair_price - 1)
        except ValueError:
            best_bid_fair = fair_price - 1
