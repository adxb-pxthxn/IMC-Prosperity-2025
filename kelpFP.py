...
class Trader:
    ...
    def ema(self, series: list[float], alpha: float) -> float:
        if not series:
            return 0
        ema_val = series[0]
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            if product == "KELP":
                if len(history) < ema_window + 2:
                    result[product] = []
                    continue

                ema_now = self.ema(history[-ema_window:], alpha)
                ema_prev = self.ema(history[-ema_window - 1:-1], alpha)
                predicted_price = ema_now + (ema_now - ema_prev)
                ...
                # RESIN stays as-is with ladder + tight MM
